...
def lyne_hollick_filter(flow_series: pd.Series, alpha: float = 0.925, passes: int = 3, n_reflect: int = 30) -> pd.DataFrame:
    """
    ...
    flow: np.ndarray = flow_series.to_numpy()
    ...
    start_pad: np.ndarray = np.flip(flow[:n_reflect])
    end_pad: np.ndarray = np.flip(flow[-n_reflect:])
    reflected_flow: np.ndarray = np.concatenate([start_pad, flow, end_pad])

    # --- 2. Filtering Passes ---
    quick_flow: np.ndarray = np.zeros_like(reflected_flow)

    # First pass (forward)
    for i in range(1, len(reflected_flow)):
        term1: float = alpha * quick_flow[i-1]
        term2: float = ((1 + alpha) / 2) * (reflected_flow[i] - reflected_flow[i-1])
        quick_flow[i] = term1 + term2

    # Enforce constraints
    quick_flow = np.maximum(0, np.minimum(quick_flow, reflected_flow))
    base_flow: np.ndarray = reflected_flow - quick_flow
